...
export interface CodeBlock {
  instanceId: string;
  type: 'move_forward' | 'turn_left' | 'turn_right' | 'jump' | 'repeat' | 'play_sound' | 'say_hello';
  label: string;
  category: 'motion' | 'looks' | 'sound' | 'events' | 'control';
  blockClass: string;
  stepValue?: number;
  repeatCount?: number;
...
export const BlocklyEditor: React.FC<BlocklyEditorProps> = ({
  ...
  const [modalSize, setModalSize] = useState({ width: 480, height: 500 });

  // Adjustable Dividing Point Percentages
  const [topSectionPercent, setTopSectionPercent] = useState<number>(42);
  const [heroesPercent, setHeroesPercent] = useState<number>(34);

  const characterList = [
    { id: 'monkey', name: 'MONKEY', avatar: '/monkey1.svg' },
  ];

  const blockPalette: Array<Omit<CodeBlock, 'instanceId'>> = [
    { type: 'move_forward', label: 'move forward', category: 'motion', blockClass: 'block-motion', stepValue: 10, icon: <IconArrowRight className="w-3.5 h-3.5" /> },
    { type: 'turn_left', label: 'turn left', category: 'motion', blockClass: 'block-motion', stepValue: 90, icon: <IconArrowBackUp className="w-3.5 h-3.5" /> },
    { type: 'turn_right', label: 'turn right', category: 'motion', blockClass: 'block-motion', stepValue: 90, icon: <IconArrowForwardUp className="w-3.5 h-3.5" /> },
    { type: 'jump', label: 'jump over', category: 'motion', blockClass: 'block-motion', stepValue: 2, icon: <IconCornerUpRight className="w-3.5 h-3.5" /> },
    ...
    { type: 'repeat', label: 'repeat until obstacle', category: 'control', blockClass: 'block-control', repeatCount: 3, icon: <IconRepeat className="w-3.5 h-3.5" /> },
  ];
  ...
  const handleHorizontalSplitPointerDown = (e: React.PointerEvent) => {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      const newPercent = Math.max(20, Math.min(65, Math.round((relativeX / rect.width) * 100)));
  ...
  return (
    ...
    >
      ...
      {/* Main Workspace Content with Content-Wrapped Block Widths */}
      {!isCollapsed && (
        <div className="pt-3 flex flex-col flex-1 min-h-0 space-y-2 w-full h-full relative">
          ...
          >
            ...
            <div className="flex-1 flex flex-col space-y-1.5 h-full overflow-hidden">
              <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest px-1">BLOCK PALETTE</span>
          ...
          {/* Interlocking Code Stack Dropzone with CONTENT-WRAPPED BLOCK WIDTHS */}
          <div className="flex-1 min-h-[90px] overflow-y-auto flex flex-col space-y-2 p-3 bg-white/50 rounded-2xl border border-slate-300/80 w-full shadow-inner items-start">
            ...
            {/* Green Flag Event Header Block WRAPPED TO CONTENT */}
            <div className="px-3.5 py-1.5 h-8 max-h-8 shrink-0 w-fit max-w-fit self-start rounded-full block-events text-slate-950 font-black text-xs shadow flex items-center space-x-2 border border-amber-600/40 select-none">
              <span>WHEN</span>
              <span className="w-4 h-4 rounded-full bg-emerald-500 flex items-center justify-center text-white shadow">
                <IconFlag className="w-3.5 h-3.5 fill-current" />
              </span>
              <span>CLICKED</span>
            </div>

            {/* Interlocking Code Blocks WRAPPED TO CONTENT */}
            <AnimatePresence>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      {block.category === 'motion' && block.stepValue !== undefined && (
                        <div className="flex items-center space-x-1">
                          <input
                            type="number"
                            value={block.stepValue}
                            onChange={(e) => handleUpdateStepValue(block.instanceId, parseInt(e.target.value) || 0)}
