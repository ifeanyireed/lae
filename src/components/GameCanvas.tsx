  ...
  return (
    <div className="w-full h-full min-h-screen fixed inset-0 z-10 flex flex-col justify-between overflow-hidden font-sans bg-[#0d0906]">
      ...
      >
        ...
        >
          {/* Custom Fixed Full-Screen Maze Artwork (full_maze.jpeg) */}
          <Image 
            src="/full_maze.jpeg" 
            alt="Full Screen Widescreen Maze Artwork" 
            fill 
            unoptimized
          ...
          {/* Connected Waypoint Track Path Polyline Overlay */}
          ...
          {liveWaypoints.map((wp) => {
            ...
            return (
              ...
              >
                ...
                {/* Character Monkey Sprite */}
                {isPlayerHere && (
                  <motion.div
                    layoutId="board-explorer-sprite"
                    transition={{ type: 'spring', stiffness: 300, damping: 25 }}
                    className="absolute inset-0 flex items-center justify-center z-[100] pointer-events-none"
                  >
                    {/* Organic Amoebaic Glass Thought Bubble with Reduced Blur & Trailing Amoebaic Circles */}
                    <AnimatePresence>
                      {speechBubble && (
                        <motion.div
                          initial={{ opacity: 0, y: 10, scale: 0.8 }}
                          animate={{ opacity: 1, y: -88, scale: 1 }}
                          exit={{ opacity: 0, scale: 0.8 }}
                          className="absolute -top-16 bg-white/85 backdrop-blur-md text-slate-800 text-xs px-5 py-2 rounded-full border border-white/50 whitespace-nowrap z-[110] pointer-events-auto flex items-center justify-center relative shadow-sm"
                        >
                          ...
                          <span className="text-slate-700 font-medium tracking-wide">{speechBubble}</span>
                        </motion.div>
                      )}
                    </AnimatePresence>

                    {/* Monkey 1 Sprite */}
                    <div className="relative transform hover:scale-110 transition z-[100]">
            ...
            >
              ...
              <div className="flex items-center space-x-2" onPointerDown={(e) => e.stopPropagation()}>
                ...
                <button
                  ...
                  className="bg-amber-400 text-slate-950 border border-amber-600 px-3.5 py-1 rounded-full text-xs font-black flex items-center space-x-1 shadow-sm hover:bg-amber-300 transition cursor-pointer"
                >
                  {savedSuccess ? <IconCheck className="w-3.5 h-3.5" /> : null}
                  <span>{savedSuccess ? 'Maze Saved!' : 'Save Maze'}</span>
                </button>
